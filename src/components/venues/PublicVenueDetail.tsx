...
export async function PublicVenueDetail({ venueId }: { venueId: string }) {
  const [venue, events] = await Promise.all([
    getPublishedVenue(venueId),
    getEventsAtVenue(venueId),
  ]);
  ...
  // Venues inherit the cover image from their organizer (joined in via mapVenue).
  const coverImageUrl = venue.coverImageUrl;

  const similarVenues = await getSimilarPublishedVenues(venueId, venue.city).catch(() => []);
      ...
      <div className="lg:col-span-2">
        ...
        <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-xl shadow-gray-200">
          <div className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-teal-800 px-6 py-10 sm:px-8 sm:py-14">
            ...
            {/* Dark overlay for text legibility, even when a cover image is present */}
            {coverImageUrl && (
          ...
          <div className="mt-6 grid gap-3">
            ...
            {venue.venueType && (
              <DetailRow icon={BuildingIcon} label={t(locale, "venueDetail.venueType")}>
                <p className="font-medium capitalize text-gray-900">{venue.venueType}</p>
              </DetailRow>
            )}
            {venue.capacity && (
        ...
        {similarVenues.length > 0 && (
          <div className="mt-6 rounded-[2rem] border border-gray-200 bg-white p-6 shadow-lg shadow-gray-100">
            <SectionHeader
              icon={BuildingIcon}
              eyebrow={t(locale, "venueGuest.similarVenues")}
              title={t(locale, "venueGuest.similarVenues")}
            />
            <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
              {similarVenues.slice(0, 4).map((v) => (
                <DiscoverVenueCard key={v.id} venue={v} locale={locale} />
              ))}
            </div>
          </div>
        )}
