            ...
            <div className="mt-6 grid gap-3">
              ...
              {event.endsAt && (
                <DetailRow icon={CalendarIcon} label={t(locale, "eventDetail.ends")}>
                  <p className="font-medium text-gray-900">{formatDateTime(event.endsAt)}</p>
                </DetailRow>
              )}
              <DetailRow icon={MapPinIcon} label={t(locale, "eventDetail.location")}>
                ...
                {venueAddress && <p className="text-xs text-gray-500">{venueAddress}</p>}
              </DetailRow>
            </div>
          ...
          {event.policies && Array.isArray(event.policies) && event.policies.length > 0 && (
            <div className="mt-6 rounded-[2rem] border border-gray-200 bg-white p-6 shadow-lg shadow-gray-100">
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                {(event.policies as Array<{ title: string; description: string }>).map(
                  (policy, i) => (
                    <div key={i} className="rounded-3xl border border-gray-100 bg-gray-50 p-5">
                      <h3 className="text-sm font-semibold uppercase tracking-[0.18em] text-gray-500">
                        {policy.title}
                      </h3>
                      <p className="mt-3 text-sm leading-7 text-gray-700">{policy.description}</p>
                    </div>
                  ),
                )}
              </div>
              ...
              <div className="mt-4 space-y-4">
                {ticketTypes.map((tt) => (
                  ...
                  >
                    ...
                    {tt.description && (
                      <p className="mt-3 text-sm text-gray-600">{tt.description}</p>
                    )}
                  </div>
                ))}
              </div>
